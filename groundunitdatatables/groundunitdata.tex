...
A ground-combat unit represents a combat formation of infantry, tanks, IFVs, APCs, trucks, tracked carriers, or artillery. Its primary role is combat with other ground units.
...
\note{
    For the Oerlikon GDF battery: the AAA values are from \emph{Air Strike} with the hit rolls reduced from 5/4/3 to 4/3/2 as described in the extended note above.
}
...
\note{
    Launcher and SAM values are from \emph{Air Strike}.
}
...
\note{
    For the SAM-7A/B: The launcher and SAM values are from \emph{Air Strike}.
}
...
9K310 Igla-1 (SA-16 Gimlet) and 9K38 Igla (SA-18 Grouse) use very similar missiles and launchers, but the Igla-1 has a single-channel IR seeker and the Igla uses a dual-channel IR seeker to reduce its susceptibility to flares. Both incorporate IFF interrogators.%
\note{
    For the SAM-16/18: The launcher and SAM values are from Malcom Pipes, but the lock-on roll for the SA-18 is changed from 8 to 7.
}
...
\note{
    For the FIM-92: The launcher and SAM values for the A model are from \emph{Air Strike}. Those for the B/C/D models are from Malcolm Pipes compendium, but the lock-on roll is changed from 8 to 7.
}
