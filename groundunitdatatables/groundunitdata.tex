...
A ground unit has a numerical damage resilience, which is the amount of damage it can suffer before being killed\footnote{Air Strike has a form of this, units whose counters have white text being only able to take 1D of damage before being killed (see the last paragraph of rule 28). These include infantry SAMs, armored CCUs, and many vehicle SAMs. TSOH also mentions units being able to take only 1D damage, but it doesn't have any relevant units since it attaches SAM teams to infantry platoons rather than having them as separate (see below) and doesn't have a separate FAC unit. In both cases, the idea seems to be have been incompletely implemented in the rule. I'm generalizing and homogenizing the idea. I need sections for mobile AAA units with two vehicles and for the Oerlikon GDF (as Skyguard can only control two guns and not a full battery). To a large degree, a section is equivalent to a platoon that has taken 1D and a squad is equivalent to a platoon that has taken 2D, so the mechanics of the game are really not impacted.}. It is determined by the size of the unit:
\begin{itemize}
...
A ground unit has a mobility that may restrict the hexes it may occupy according to the terrain in an around those hexes. The scenario may modify these restrictions.\footnote{The inspiration for this rule was \href{https://commons.wikimedia.org/wiki/File:Aerial_view_of_the_road_through_Mitla_Pass_dotted_with_wrecked_Egyptian_vehicles_and_armour_after_Israel_air_force_attacks._June_1967._D326-013.jpg}{this image} of the Mitla Pass from 1967. Also, think of the Falklands; the terrain was open, but impassable to everything except the CVR(T)s and Bv~202s. The basic rule is effectively: don't put trucks or towed weapons in forests unless there is a road. The advanced rule is effectively: if the terrain is difficult, keep your trucks and towed weapons near roads even in clear terrain. The rule only applies if the scenario allows freedom to place the units; if it specifies their positions then it takes precedent. I note that this rule is not infallible: in the Falklands War the Panhard AMLs were restricted to roads and trails, and this corresponds to poor mobility but under this rule they would still have unrestricted mobility. A scenario could fix this problem by explicitly giving them poor mobility.}
...
An infantry or towed unit may be \emph{transported} by a specific IFV, APC, truck, or tracked-carrier \emph{transport} unit.\footnote{The idea of transporting units comes from the “Thunder and Fire” scenario we're currently playing. The rules here simply generalize it slightly, incorporate the idea of mobility, and add rules for mounting and dismounting.}
