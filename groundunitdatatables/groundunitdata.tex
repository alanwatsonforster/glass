...
The \emph{attached} sections are commonly infantry SAM or FAC sections. A scenario may add further restrictions on forming composite platoons.\footnote{This rule comes from TSOH. It doesn't have counters for infantry SAMs, so in the Easter Offensive scenarios it says treat infantry platoons as if they have an infantry SAM squad. This rule helps with stacking (since attached squads do not count for stacking) and with keeping your precious FACs and SAM teams anonymous. It also works nicely with the transport rule, since attached squads do not need their own transport but can hop a ride with the platoon.}
...
\paragraph{Attached Sections.}

An attached section is not represented by a separate counter, does not count for stacking, and may not be attacked directly.
Instead, it is considered to form part of its composite platoon.
An attached section retains its normal capabilities (i.e., an attached SAM section may engage aircraft and an attached FAC section may sight and mark targets).

If a composite platoon is suppressed, each attached section is suppressed too.
If a composite platoon suffers damage, then for each D of damage, roll one die for each attached section and on a \minusafter{3} the attached section is killed.
If a composite platoon is killed, each attached section is killed too.
...
Infantry SAM and heavy SAM squads are described in more detail a subsequent section. Their VP values depend on their type of SAM.
An infantry FAC and SAM section will often be attached to an infantry, infantry weapons, or infantry HQ platoon or transported by a truck section.
...
An anti-tank vehicle is a lightly armored vehicle equipped with ATGMs.\footnote{Again, anti-tank units are mainly chrome. However, in ground combat we might give them more effectiveness against vehicular or towed targets and less against infantry targets.}
...
\begin{itemize}\raggedright
    ...
    \item Open Tanks: ASU-57 and SU-76.\footnote{Open tanks are pretty uncommon, but we need open target class for M3 half-track APCs, so it is easy to apply it here too.}
...
An armored or infantry HQ platoon represents the HQ unit of a battalion or regiment.\footnote{I wondered if HQ platoons were also used at the company level, but looking at the Air Strike scenarios they are included at a ratio of about one HQ platoons to nine normal platoons, so they seem to be battalion HQs.} There are no IFV, APC, or truck HQ units; the HQ platoon of a mechanized or motorized infantry battalion or regiment will consist of either an armored HQ platoon or an infantry HQ platoon associated with a normal IFV, APC, or truck platoon.
...
Table~\ref{table:aaa-units} summarizes the properties of AAA units\footnote{I've added most from Air Strike, EOG, and TSOH. I didn't add the Blazar, Gemag, or Nile 23 since they didn't make it to service, but instead substituted the similar LAV-AD and Sinai 23 which did.}, and Figure~\ref{figure:aaa-units} shows their representation as counters.
...
The Sinai 23 has a Dassault TA20 turret with two 23 mm guns and six Ayn-al-Saqr (similar to the Strela-2M) or Stinger missiles on an M113 chassis. It has night IR sights. Each battery operates with one vehicle equipped with an RA20S search radar.
