...
A ground \emph{unit} is a combat formation of infantry, armor, transports, AAA, SAM launchers, or radars. This section discusses them in general.%
\footnote{
...
A ground unit has a mobility that may restrict the hexes it may occupy according to the terrain in an around those hexes.
...
One common modification is to explicit indicating the hexes occupied by each ground unit; in this case this rule is largely superfluous.%
\footnote{
...
\begin{figure}
    ...
    \caption{Mobility. {\normalfont A ground unit with poor mobility may only occupy the hexes marked with green dots; one with good mobility may occupy those marked with yellow or green dots; and one with unlimited mobility may occupy those marked with red, yellow, or green dots. No ground unit may occupy the hex marked with a blue dot.}}
...
A mounted unit or mounted cargo is not represented by a separate counter, does not count for stacking, and may not be attacked directly.
Instead, it is considered to form part of its transport unit.
If its transport unit suffers damage, then a mounted unit or cargo suffers the same damage.
...
\footnote{
    Being placed according to the mobility of its transport allows a tracked transport can place a towed unit in a forest hex.
}
...
A mechanized infantry platoon will normally be transported by platoon of APCs or IFVs.
...
\begin{itemize}\raggedright
    ...
    \item Open Tanks: ASU-57 and SU-76.%
        \footnote{
            Open tanks are uncommon, but we need open target class for M3 half-track APCs, so it is easy to apply it here too.
        }
...
AAA units are specialized in the use of guns to destroy attacking aircraft.
...
\footnote{
    This note summarized how I have determined the AAA data values for AAA units. VP values are discussed separately.

    The upper part of the following table shows the original data for gun batteries from \emph{Air Strike} (AS), \emph{Eagles of the Gulf} (EOTG), and \emph{The Speed of Heat} (TSOH). I have added columns with the caliber, burst firing rate per mount, and muzzle velocity.
    ...
    \begin{tabular}{lrlllllrrl}
        ...
        &\multicolumn{1}{c}{\vertical{Rate/mount (RPM)}}
        &\multicolumn{1}{c}{\vertical{Velocity (m/s)}}
        ...
        61-K (M-38)&&&2/2/1\\
        Bofors L/60&&&2/2/1\\
        Bofors L/70&16&4/9/13&3/2/1\\
        \bottomrule
    \end{tabular}
    \medskip

    We can see that the basic trend is that altitude and range increase with caliber, hit rolls increase with firing rate, and damage rating increases with both caliber and rate of fire. This is more or less as expected, although we might also consider that the 10\% variation in muzzle velocity might have a small effect on the range and hit rolls (as the rounds fall less during flight and less lead needs to be pulled).

    Most of the values can be used as they are, but there are a few anomalies in the hit rolls that need commenting upon or adjusting:
    \begin{itemize}
        \item The M167 and Gemag have hit rolls of 6/5/4 and the Rh-202 has 5/4/3. Otherwise, they are similar, except that the M167 and Gemag have FCR-R and the Rh-202 does not (and the M167 has a moderate advantage in rate of fire). I consider the FCR-R of the M167 and Gemag to be largely responsible for increasing their hit rolls by \plus{1} compared to the Rh-202.
        \item The ZU-23 has unexpectedly low hit rolls of 4/3/2 for its rate of fire. The values of 5/4/3 for the Rh-202, which has a similar rate of fire, are probably more appropriate.
        \item The Oerlikon GDF has unexpectedly high hit rolls of 5/4/3 for its rate of fire. The values of 4/3/2 for the ZPU-2, which has a similar rate of fire, are probably more appropriate. (This reduction also makes the Oerlikon GDF more consistent with Gepard.)
        \item The hit rolls of 3/3/2 of the Bofors L/70 are unexpectedly low compared to those of 3/2/1 of the 61-K (M-38) and L/60. This case is complicated, because the obvious solution of giving the Bofors L/70 hit rolls of 4/3/2 would make it have the same values as the Oerlikon GDF, despite its much lower rate of fire. A better solution is probably to adjust the 61-K (M-38) and L/60 down to 2/2/1, which matches the S-60, and the L/70 to 3/2/1, and then increase the ranges of the L/70 from 4/8/12 to 4/9/13, giving it a slight advantage both at shorter and longer ranges compared to the L/60.
        \item Again, the L/70 has the same maximum altitude as the L/60, despite its higher muzzle velocity. A value of 16 is probably more appropriate.
    \end{itemize}

    Comparing similar weapons in Air Strike, EOTG, and TSOH, batteries (with four to six mounts) typically have a \plus{1} advantage on their hit rolls compared to sections (with two mounts).
    In a very few cases, the difference is 0 and in a very few others, it is \plus{2}.
    The ranges and damage ratings tend to be the same.
    I have adopted \plus{1} uniformly.

    The basis for the values I will adopt for AAA units is:
    \begin{itemize}
        \item The values in the table above, with modifications.
        \item An adjustment of \minus{1} to the hit rolls for sections.
        \item An adjustment of \plus{1} to the hit rolls for FCR-R or similar.
        \item An adjustment of \plus{2} to the hit rolls for FCR-W.
    \end{itemize}
...
\footnote{
    For the M2 platoon: I have adopted the Air Strike values.
}
...
The DShK-38 12.7 mm heavy machine guns is most commonly found on a low, wheeled mount for ground combat, but here is used on a tall tripod suitable for air defense. A platoon has six guns.%
\footnote{
    For the DShk-38 platoon: I have adopted the Air Strike values.
}
...
\footnote{
    For the M16 section: I have adopted the EOG values.
}

The M16 was used by United States forces during World War II and (primarily in an anti-personnel weapon) in the Korean War for airfield defense. Additionally, Israeli forces employed it during the 1956 Arab-Israeli War.
...
\footnote{
    For the M55 battery: I have adopted the EOG values.
}
...
\footnote{
    For the ZPU-1 battery: Puma gives the long range as 4 hexes, but EOG and TSOH give it as 5 hexes. Furthermore, Puma gives the long range for the ZPU-2 and ZPU-4 as 5 hexes. I have adopted the EOG and TSOH values.
}%
\footnote{
    For the ZPU-2 battery: Air Strike has hit rolls of 4/3/2 and a damage rating of 1. Puma has hit rolls of 4/3/1 and a damage rating of 3. Comparing to the ZPU-1 and ZPU-4, the Air Strike values seem to be more reasonable, and I have adopted it.
}%
\footnote{
    For the ZPU-4 battery: Air Strike and Puma have hit rolls of 5/4/3. TSOH has hit rolls of 4/3/2, which matches the ZPU-2 in Air Strike and Puma. I have adopted the Air Strike values.
}
...
\footnote{
    For the mobile ZPU-4 section: Air Strike has hit rolls of 3/3/2. Puma has hit rolls of 5/4/3. The Puma values are equal to those of as ZPU-4 \emph{battery}. The Air Strike values might be more appropriate for a \emph{section}. I have chosen to use 4/3/2 obtained by applying the \minus{1} battery-to-section modifier to the values for a ZPU-4 battery. Air Strike gives this 5 VPs, which is the same as the towed ZPU-4 battery despite the mobile section having worse hit rolls and defense strength. I think 4 VPs (i.e., one less than the battery) is more appropriate.
}%
\footnote{
    For the mobile ZPU-1 and ZPU-2 section: These do not appear in any of the Air Power sources, but they are very common. I have chosen to apply the \minus{1} battery-to-section modifier to the hit roll and VP values for a ZPU-1 and ZPU-2 batteries.
}
...
\footnote{
    For the M167 platoon: I have used the Air Strike values. I'm not sure when it gained FCR-R or if it gained night IR sights.
}
...
The M163 Vulcan Air Defense System (VADS) has a 20 mm six-barreled Vulcan gun in a lightly armored but open-topped turret on an adapted M113 APC. The fire-control system has optical sights and a range-only radar. A section has two vehicles.\footnote{
    For the M163 section: I have used the Air Strike values. I have the same uncertainties as I do for the M167.
}
...
\footnote{
    For the TCM-2 battery: I have adopted the EOG values.
}
...
\footnote{
    For the M3 TCM-2 section: I have adopted the EOG values.
}
...
\footnote{
    For the Rh-202 battery: Air Strike also has a single-mount version, but I can find no evidence for this other than as the main weapon of IFVs (e.g., the Marder) or on naval mounts. All AAA mounts seem to be twin. I adopt the Air Strike values for the twin mount.
}
...
\footnote{
    ...
    Air Strike gives hit rolls of 3/3/2, which are identical to those of the M3 TCM-20 section. That would make sense if both had similar fire control systems. However, the Panhard M3 DCA  radar-equipped vehicle seems to be able to search and share speed and range information with the other vehicle in the section. That seems closest to FCR-R. Adding \plus{2} for a FCR would give hit rolls of 5/5/4 which is better than the 5/4/3 of the ZSU-23-4 and 5/4/3 of the Gepard! A modifier of \plus{1} seems more appropriate, giving hit rolls of 4/4/3.
}
...
\footnote{
    For the ZU-23 battery: I adopt the Air Strike and TSOH values. However, TSOH gives it 6 VPs and Air Strike gives it only 5. I think 6 is more appropriate, comparing to the ZPU-4 battery.
}
...
\footnote{
    For the ZU-23 section: Air Strike gives this hit rolls of 2/2/1 and 5 VPs. Applying the \minus{1} modifier to the ZU-23 battery would give 3/2/1, which seems more appropriate. The VPs seem about right, as the battery has 6 VPs.
}
...
\footnote{
    For the Sinai 23 section: The turret seems to be similar to the LAV-AD turret, so I've adopted the same hit rolls. The range and damage rolls are adopted from the mobile ZU-23 section. I don't know which model of Stinger was used; I would guess the FIM-92A.
}
...
\footnote{
    For the ZSU-23-4 section: The values are adopted from Air Strike.
}
...
\footnote{
    For the LAV-AD section: The system seems similar to the Blazer, but without radar and with night IR sights. I've adopted the range and damage rating for the Blazer, and the hit rolls of 4/3/2 which corresponds to those of the M163 with a \minus{1} modifier for not having FCR-R.
}
...
\footnote{
    For the M53/59 section: The hit rolls of 4/3/2 in Air Strike seem a little high. A modern 23/30/35 mm section with FCR typically has 5/4/3. I adopt 3/2/1, matching the mobile ZU-23 and the AMX-30 and Tunguska without FCR. If the VADS is classed as armored, should this be armored too?
}
...
\footnote{
    For the Tunguska section: I have adopted the Air Strike values for the ZSU-30-2 and added missiles. The VP values are tentative, and might be modified once other SAM launchers are considered.
}
...
\footnote{
    For the Tunguska-M section: I have adopted the Air Strike values for the ZSU-30-2 and added missiles. The VP values are tentative, and might be modified once other SAM launchers are considered.
}
...
\footnote{
    For the Gepard section: I have adopted the Air Strike values.
}
...
\footnote{
    For the 61-K battery: This is what Air Strike, EOG, and TSOH call the “M-38”, but I have no idea why. Its other designation is M-1939, so I could understand “M-39“. I have adopted the values from Air Strike, which are the same as those in Air Strike, but TSOH gives it 6 VPs and Air Strike only 5.
}
...
\footnote{
    For the Bofors L/60 battery: EOG gives hit rolls of 2/2/1 and 5 VPs. I have adjusted the hit rolls to 3/2/1 to match the very similar 37 mm 61-K.
}
...
\footnote{
    For the Bofors L/70 battery: I have adopted the values from Air Strike, although I note they do not reflect the reported greater range of the L/70 compared to the L/60.
}
...
\footnote{
    For the BOFI and BOFI-R battery: I've not been able to find much information. I take hit rolls for the L/70 and add +1 for proximity fuzes, +1 for BOFI laser rangefinder (effectively FCR-R), and another +1 for BOFI-R radar.
}
...
\footnote{
    For the S-60 battery: I have adopted the values from TSOH. These are the same as in Air Strike, except that TSOH gives 8 VPs and Air Strike only 6.
}
...
\footnote{
    For the ZSU-72-2 section: I have adopted the EOG hit rolls of 2/1/1. The S-60 battery has 2/2/1 and the normal -1 modifier would suggest the ZSU-57-2 have 1/1/0, but in this case a smaller difference is justified as we are comparing four barrels to six.
}
