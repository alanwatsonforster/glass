\section{Shenyang JJ-2 and FT-2}

\includegraphics[width=\linewidth]{MiG-15UTI.png}

The Shenyang JJ-2 was a licensed copy of the MiG-15UTI trainer. The MiG-15UTI in turn was essentially a MiG-15 with a second cockpit and reduced armament. The NATO reporting name is Midget.

\subsection{Versions}

\subsubsection{JJ-2}

The JJ-2 was the domestic version.

It was used extensively by the PLAAF and PLAN.

\subsubsection{FT-2}

The FT-2 was the export version of the JJ-2.

\subsection{Armament and Stores}

It is conceivable that the JJ-2 used the same stores as the MiG-15, especially 250L FTs.

\subsection{Combat}

The PLAN used the JJ-2 to attempt to intercept ROCAF overflights around 1960.

\subsection{ADCs}
\begin{adclist}
    \adcitem{Shenyang JJ-2}
    \adcitem{Shenyang FT-2}
\end{adclist}

See also:
\begin{typelist}
    \typeitem{Mikoyan-Gurevich MiG-15}
\end{typelist}

\subsection{Photo Credit}
\begin{itemize}\raggedright
        % I can't find a photo of a real JJ-2.
    \item \href{https://commons.wikimedia.org/wiki/File:Duxford_Air_Festival_2017_-_mig1_(34842016051).jpg}{MiG-15UTI}: wallycacsabre (CC-BY-2.0)
\end{itemize}
