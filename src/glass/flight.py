...
def _move(E, flighttype, power, moves, **kwargs):
    ...
    E.logcomment("speed of sound is %.1f." % glass.speed.m1speed(E.altitudeband()))

    if E.isaircraft():
        if E.geometry() is not None:
            E.logstart("geometry         is %s." % E.geometry())
        glass.speed._startaircraftspeed(E, power, **kwargs)
    else:
        glass.speed._startmissilespeed(E)
...
def _checknormalflight(E):
    ...
    if E._flighttype[:4] == "HRD/":
        ...
        # See rule 7.7.
        if E.altitude() > glass.capabilities.ceiling(E):
            E.logcomment(
    ...
    if E._flighttype == "LVL":
        ...
        if E.speed() < glass.capabilities.minspeed(E) + 1:
...
def _checkstalledflighttype(E):
    ...
    if E.speed() >= glass.capabilities.minspeed(E):
        raise RuntimeError("flight type cannot be ST as aircraft is not stalled.")

    E.logstart("speed is below the minimum of %.1f." % glass.capabilities.minspeed(E))
...
def _startmove(E, **kwargs):
    ...
    E._m1speed = glass.speed.m1speed(E.altitudeband())
...
def _startmovenormalflight(A):
    ...
    def checkcloseformationlimits():
        ...
        if A._hrd:
            ...
            glass.closeformation.breakdown(A)
    ...
    def determinefprequirements():
        ...
        if A._flighttype == "LVL":
            ...
            # See rule 8.1.2.
            if A.speed() < glass.capabilities.minspeed(A) + 1.0:
                raise RuntimeError("insufficient speed for SC.")
            climbcapability = glass.capabilities.climbcapability(A)
            if A.speed() < glass.capabilities.climbspeed(A):
                climbcapability /= 2
    ...
    def handlecarriedturn():
        ...
        if _isturn(A._maneuvertype):
            ...
            A._maneuversupersonic = A.speed() >= glass.speed.m1speed(A.altitudeband())
    ...
    def determineeffectiveclimbcapability():
        ...
        if _isclimbingflight(A._flighttype):
            ...
            A._effectiveclimbcapability = glass.capabilities.climbcapability(A)

            # See rule 4.3 and 8.1.2.
            if A._flighttype == "SC" and A.speed() < glass.capabilities.climbspeed(A):
            ...
            if not glass.variants.withvariant(
            ...
            if A.speed() >= glass.speed.m1speed(A.altitudeband()):
...
def _endmissileflight(M):
    glass.speed._endmissilespeed(M)
    ...
    def determinemaxturnrateap():
        ...
        if glass.variants.withvariant("use house rules"):
            ...
            if A._maxturnrate != None:
                A._turnrateap = -glass.capabilities.turndrag(A, A._maxturnrate)
            else:
                A._turnrateap = 0
...
def _domove(E, move, actiondispatchlist):
    ...
    def checkminspeed():
        ...
        if E.isaircraft():
            ...
            E._minspeed = glass.capabilities.minspeed(E)
    ...
    E._movestartm1speed = glass.speed.m1speed(E.altitudeband())
    ...
    if E.ismissile() and E.speed() < glass.speed.missileminspeed(E.altitudeband()):
    ...
    try:
        ...
        if remainingactions != actions:
            ...
            assert glass.hex.isvalid(E.x(), E.y(), facing=E.facing())
            assert glass.altitude.isvalidaltitude(E.altitude())
        ...
        if glass.variants.withvariant("use house rules") and E._maneuvertype == "SL":
            E._maneuverfp += 1
...
def _checkmaneuveringdeparture(A):
    ...
    if A._hasmaneuvered and A._hasrolled:
        if A._movestartaltitude > glass.capabilities.ceiling(A):
            A.logcomment(
                "check for a maneuvering departure as the aircraft is above its ceiling and attempted to roll."
            )
    ...
    if A._hasmaneuvered and A._hasturned:
        if (
            A._movestartaltitude > glass.capabilities.ceiling(A)
...
def _checkcloseformation(A):
    # See rule 7.8.
    if A._hasturned and glass.closeformation.size(A) != 0:
        if (
            (glass.closeformation.size(A) > 2 and A._movemaneuvertype == "HT")
    ...
    # See rule 13.7, interpreted in the same sense as rule 7.8.
    if A._hasrolled and glass.closeformation.size(A) != 0:
...
def _doclimb(E, altitudechange):
    ...
    # See rule 8.5.
    if E._flighttype == "SC" and E.altitude() > glass.capabilities.ceiling(E):
...
def _checkturnrate(A, turnrate):
    ...
    # See rule 7.1.
    turnrateap = glass.capabilities.turndrag(A, turnrate)
    if turnrateap == None:
...
def _dodeclareturn(E, turnrate, sense):
    ...
    if E.isaircraft():
        ...
        turnrateap = glass.capabilities.turndrag(E, turnrate)

    else:

        if E.speed() < glass.speed.missilemaneuverspeed(E.altitudeband()):
            raise RuntimeError(
...
def _extrapreparatoryhfp(E):
    ...
    if E.speed() >= glass.speed.m1speed(E.altitudeband()):
...
def _dodeclaredisplacementroll(E, sense):
    ...
    if E.hasproperty("NRM"):
        raise RuntimeError("aircraft cannot perform rolling maneuvers.")
    if glass.capabilities.rolldrag(E, "DR") == None:
    ...
    if glass.variants.withvariant("use house rules"):
        E._othermaneuversap -= glass.capabilities.rolldrag(E, "DR")
        if E._maneuversupersonic:
            if E.hasproperty("PSSM"):
                E._othermaneuversap -= 1.0
            elif not E.hasproperty("GSSM"):
...
def _dodeclarelagroll(E, sense):
    ...
    if E.hasproperty("NRM"):
        raise RuntimeError("aircraft cannot perform rolling maneuvers.")
    if glass.capabilities.rolldrag(E, "LR") == None:
    ...
    # See rules 13.3.1 and 6.6.
    if glass.variants.withvariant("use house rules"):
        E._othermaneuversap -= glass.capabilities.rolldrag(E, "LR")
        if E._maneuversupersonic:
            if E.hasproperty("PSSM"):
                E._othermaneuversap -= 1.0
            elif not E.hasproperty("GSSM"):
...
def _dodeclareverticalroll(E, sense):
    ...
    if E.isaircraft():
        ...
        if glass.variants.withvariant("use house rules"):
            E._othermaneuversap -= glass.capabilities.rolldrag(E, "VR")
            if E._maneuversupersonic:
                if E.hasproperty("PSSM"):
                    E._othermaneuversap -= 1.0
                elif not E.hasproperty("GSSM"):
...
def _dodeclaremaneuver(E, maneuvertype, sense, continuedturn=False):
    ...
    if E.isaircraft() and glass.variants.withvariant("use house rules"):
