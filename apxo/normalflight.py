...
def checkflight(A):
    ...
    if hrd:
        if previousflighttype == "LVL" and flighttype == "VD":
            ...
            previousflighttype == "ZC" or previousflighttype == "SC"
        ) and flighttype == "VD":
            pass
    ...
    if flighttype == "LVL":
        ...
        if previousflighttype == "VC" and not (
...
def continueflight(A, tasks, note=False):
            ...
            if flighttype == "SD":
                ...
                if altitudechange != 2 and altitudechange != 3:
                    raise RuntimeError(
                        ...
                        % plural(
                            altitudechange, "1 level", "%d levels" % altitudechange
                        )
                    )
    ...
    def dodeclareturn(A, turnrate, sense):
        ...
        if flighttype == "VC" or flighttype == "VD":
            raise RuntimeError(
                "attempt to declare turn while flight type is %s." % flighttype
            )
    ...
    def doturn(A, sense, facingchange, continuous):
        ...
        # See rule 7.1.
        if (
            A._maneuverfp < A._maneuverrequiredfp
            or facingchange > A._maneuverfacingchange
        ):
    ...
    def dodeclareslide(A, sense):
        ...
        if flighttype == "VC" or flighttype == "VD":
            raise RuntimeError(
                "attempt to declare slide while flight type is %s." % flighttype
            )
    ...
    def dodeclaredisplacementroll(A, sense):
        ...
        if apvariants.withvariant("use version 2.4 rules"):
            ...
            A._maneuverrequiredfp = (
                apcapabilities.rollhfp(A) + extrapreparatoryhfp() + 1
            )
    ...
    def dodeclarelagroll(A, sense):
        ...
        if flighttype == "SC" or flighttype == "VC" or flighttype == "VD":
            raise RuntimeError(
                "attempt to declare a lag roll while flight type is %s." % flighttype
            )
    ...
    def dossgt(A, m):
        ...
        if useofweaponsforbidden(A):
            raise RuntimeError(
                "attempt to start SSGT while %s" % useofweaponsforbidden(A)
            )
        ...
        if apairtoair.trackingforbidden(A, target):
            raise RuntimeError(
                ...
                % apairtoair.trackingforbidden(A, target)
            )
...
def startflight(A, actions, note=False):
    ...
    def checkcloseformationlimits():
        ...
        if (
            flighttype == "ZC"
            or (flighttype == "SC" and A._powersetting == "AB")
            or flighttype == "VC"
            or flighttype == "UD"
            or flighttype == "VD"
        ):
            A._logevent(
                "close formation breaks down as the flight type is %s." % flighttype
            )
            apcloseformation.breakdown(A)
    ...
    def determinefprequirements():
        ...
        elif previousflighttype == "LVL" and (
            _isclimbingflight(flighttype) or _isdivingflight(flighttype)
        ):
            mininitialhfp = 1
        elif (
            _isclimbingflight(previousflighttype) and _isdivingflight(flighttype)
        ) or (_isdivingflight(previousflighttype) and _isclimbingflight(flighttype)):
...
def _aftertask(A):
    ...
    if A._hasmaneuvered and A._hasturned:
        if A._taskaltitude > apcapabilities.ceiling(A) and A._taskmaneuvertype != "EZ":
            A._logevent(
